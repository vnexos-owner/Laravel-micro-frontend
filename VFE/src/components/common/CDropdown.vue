...
const filteredOptions = computed((): DropdownOption[] => {
  ...
  return props.options.filter((o) =>
    'group' in o || o.label?.toLowerCase().includes(q) || o.description?.toLowerCase().includes(q)
  )
})
...
const selectedLabel = computed((): string => {
  if (props.multiple) {
    ...
    return props.options
      .filter((o: DropdownOption): o is Extract<DropdownOption, { value: string | number }> => 'value' in o && values.includes(o.value as string | number))
      .map((o: DropdownOption) => o.label)
      .join(', ')
  }
  const found = props.options.find((o): o is Extract<DropdownOption, { value: string | number }> =>
    'value' in o && o.value === props.modelValue
  ) as DropdownOption | undefined
  return found?.label ?? ''
})

function toggle(): void { if (!props.disabled) isOpen.value ? close() : open() }
function open(): void   { isOpen.value = true; searchQuery.value = ''; focusedIndex.value = -1; emit('open') }
function close(): void  { isOpen.value = false; emit('close') }

function isSelected(item: DropdownOption): boolean {
  if (!('value' in item)) return false
  return props.multiple
    ? (Array.isArray(props.modelValue) ? props.modelValue : []).includes(item.value as string | number)
...
function onKeydown(e: KeyboardEvent): void {
  ...
  if      (e.key === 'ArrowDown') { e.preventDefault(); focusedIndex.value = Math.min(focusedIndex.value + 1, selectables.length - 1) }
  else if (e.key === 'ArrowUp')   { e.preventDefault(); focusedIndex.value = Math.max(focusedIndex.value - 1, 0) }
  else if (e.key === 'Enter' && focusedIndex.value >= 0) { e.preventDefault(); select(selectables[focusedIndex.value] as DropdownOption) }
...
<template>
  <div class="relative inline-flex w-full flex-col" ref="dropdownRef">
    <!-- Trigger -->
    <slot name="trigger" :toggle="toggle" :isOpen="isOpen" :selectedLabel="selectedLabel" :disabled="disabled">
      <button
        type="button"
        :disabled="disabled"
        ...
        :class="[sizeClasses, variantClasses, { 'border-accent ring-2 ring-focus ring-offset-2 ring-offset-background': isOpen }]"
      >
        ...
        <span class="flex items-center text-muted transition-transform duration-200" :class="{ 'rotate-180': isOpen }">
    ...
    >
      ...
      >
        <!-- Search -->
        <div v-if="searchable" class="flex items-center gap-2 px-2.5 py-1.5 mb-1 border-b border-separator">
          <svg class="text-muted shrink-0" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <!-- Header slot -->
        <div v-if="$slots.header" class="px-2.5 py-1.5 text-xs text-muted border-b border-separator mb-1">
          <slot name="header" />
        </div>

        <!-- List -->
        <ul class="flex flex-col gap-px list-none m-0 p-0">
          <li v-if="filteredOptions.length === 0" class="px-2.5 py-4 text-center text-sm text-muted">
            <slot name="empty">No options found</slot>
          </li>

          <template v-for="(item, index) in filteredOptions" :key="'value' in item ? item.value : index">
            <!-- Group label -->
            <li v-if="item.group" class="px-2.5 pt-1.5 pb-0.5 text-[11px] font-semibold uppercase tracking-widest text-muted">
              {{ item.group }}
            </li>
            ...
            <RouterLink
              ...
              :class="[
                item.disabled ? 'opacity-disabled cursor-not-allowed pointer-events-none' : 'cursor-pointer',
                isSelected(item) ? 'bg-accent/10 text-accent font-medium' : 'hover:bg-default text-overlay-foreground',
              ]"
            >
              <span :class="['flex flex-col flex-1 overflow-hidden', item.class ?? '']">
        ...
        <!-- Footer slot -->
        <div v-if="$slots.footer" class="px-2.5 py-1.5 text-xs text-muted border-t border-separator mt-1">
          <slot name="footer" />
        </div>
      </div>
    </Transition>
